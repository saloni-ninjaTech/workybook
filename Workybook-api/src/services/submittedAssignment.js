...
module.exports = {
        ...
        let list = await assignmentModel.aggregate([
         ...
         ] } },
            {
                "$lookup": {
                    ...
                    pipeline: [{$match: {$expr: {$eq: ["$student", mongoose.Types.ObjectId(studentId)]}}}],
    ...
    getByProperty: async (query, select) => {
        ...
        if (assignment && assignment.length) {
          return await assignment.map((oneAssignment) => {
                ...
                oneAssignment.contentScore.map(async (oneContentScore) => {
                    ...
                    if (oneContentScore.time) { times.push(oneContentScore.time) }
                });
                let time;
                if (times?.length) {
                    const totalDurations = times.slice(1).reduce((prev, cur) => moment.duration(cur).add(prev),
                ...
                oneAssignment.totalScore.time=time
